...
resource "newrelic_notification_channel" "slack_noise" {
  name           = "tbnl-${var.environment}-noise-channel: ${var.slack_noise_channel.name}"
  type           = "SLACK"
  destination_id = data.newrelic_notification_destination.slack.id
  product        = "IINT"
  ...
  property {
    key           = "channelId"
    value         = var.slack_noise_channel.id
    display_value = var.slack_noise_channel.name
  }
...
# Noise policy and workflow
#
resource "newrelic_alert_policy" "noise" {
  name                = "tbnl-${var.environment}-noise-policy"
  incident_preference = "PER_POLICY"
}

resource "newrelic_workflow" "noise" {
  name                  = "tbnl-${var.environment}-noise-workflow"
  enabled               = true
  muting_rules_handling = "DONT_NOTIFY_FULLY_MUTED_ISSUES"
  ...
  issues_filter {
    ...
    predicate {
      attribute = "labels.policyIds"
      operator  = "EXACTLY_MATCHES"
      values    = [newrelic_alert_policy.noise.id]
    }
  }

  destination {
    channel_id              = newrelic_notification_channel.slack_noise.id
    notification_triggers   = ["ACKNOWLEDGED", "ACTIVATED", "CLOSED"]
    update_original_message = true
  }
}
...
# resource "newrelic_nrql_alert_condition" "wwwdev_synthetic_monitor_failures" {
#   account_id = 4355133
#   policy_id = <Your Policy ID>
#   type = "static"
#   name = "www-dev synthetic monitor failures"
#   enabled = true
#   violation_time_limit_seconds = 259200

#   nrql {
#     query = "SELECT filter(count(*), WHERE result = 'FAILED') AS 'Failures' FROM SyntheticCheck WHERE entityGuid IN ('NDM1NTEzM3xTWU5USHxNT05JVE9SfDQ0YjJmMzY3LWJjMjgtNDVjNy04NzhjLTUxMzVjZjM5NjcwOA') AND NOT isMuted FACET location, monitorName"
#     query = "SELECT filter(count(*), WHERE result = 'FAILED') AS 'Failures' FROM SyntheticCheck WHERE monitorName IN ('www-test.tibobeijen.net') AND NOT isMuted FACET location, monitorName"
#   }
